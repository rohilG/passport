      ...
      <header className="w-full flex justify-between p-5 mx-auto shadow-2xl">
        <div className="mx-4 flex relative">
          ...
          <ClickOutHandler onClickOut={() => setUserDropdownVisbility("hidden")}>
            <button className='rounded-md flex ml-4 border border-blue-700' onClick={() => toggleUserDropdown() }>
              {!account.publicKey && (
            ...
            <div className={"absolute right-0 top-8 border border-blue-700 z-10 rounded-md overflow-hidden text-crypdit_text " + userDropdownVisbility}>
            {account.publicKey && (
            ...
            {!account.publicKey && (
              <button onClick={() => authModal.setShow(true)} className='block flex w-50 py-2 px-3 text-blue-700 hover:bg-blue-700 hover:text-white text-sm'>
